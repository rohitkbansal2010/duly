...
data "azurerm_api_management_product" "ch-apis" {
...
resource "azurerm_api_management_product_api" "ch-apis-product" {
  count               = length(var.api_product_ids)
  api_name            = azurerm_api_management_api.api.name
  product_id          = data.azurerm_api_management_product.ch-apis[count.index].product_id
  api_management_name = var.apim_name
  resource_group_name = var.apim_rg_name
}
