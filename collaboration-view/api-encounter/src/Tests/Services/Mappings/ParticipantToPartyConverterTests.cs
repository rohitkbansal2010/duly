// <copyright file="ParticipantToPartyConverterTests.cs" company="Duly Health and Care">
...
{
    [TestFixture]
    internal class ParticipantToPartyConverterTests : MapperConfigurator<RepositoryModelsToProcessContractsProfile>
    {
        ...
        public void ConvertTest_Practitioner_ToParty()
        {
            ...
            result.Id.Should().BeEquivalentTo(source.Person.Id);

            result.HumanName.Should().BeOfType<ApiContracts.HumanName>();
            result.HumanName.FamilyName.Should().BeEquivalentTo(source.Person.Names.First().FamilyName);
            result.HumanName.GivenNames.Should().NotBeNull();
            result.HumanName.GivenNames.Should().HaveCount(source.Person.Names.First().GivenNames.Length);
            result.HumanName.Suffixes.Should().NotBeNull();
            result.HumanName.Suffixes.Should().HaveCount(source.Person.Names.First().Suffixes.Length);
            result.HumanName.Prefixes.Should().NotBeNull();
            result.HumanName.Prefixes.Should().HaveCount(1);
            result.HumanName.Prefixes[0].Should().BeEquivalentTo(source.Person.Names.First().Prefixes.First());

            result.Photo.Should().BeOfType<ApiContracts.Attachment>();
            result.Photo.Title.Should().BeEquivalentTo(source.Person.Photos.First().Title);
            result.Photo.ContentType.Should().BeEquivalentTo(source.Person.Photos.First().ContentType);
            result.Photo.Size.Should().Be(source.Person.Photos.First().Size);
            result.Photo.Data.Should().BeEquivalentTo(source.Person.Photos.First().Data);
            result.Photo.Url.Should().BeEquivalentTo(source.Person.Photos.First().Url);
        ...
        public void ConvertTest_Throw_ServiceNotMappedException()
        {
            ...
            var source = new Participant
            {
                Person = new HumanGeneralInfoWithPhoto(),
            ...
            result.Which.Message.Should().Be($"Unmapped MemberType {source.MemberType}");
        }
    }
}
