...
namespace Duly.CollaborationView.Encounter.Api.Tests.Controllers
{
    ...
    public class PractitionersControllerTests
    {
        ...
        private const string TestSiteId = "test-site";
        private const string PractitionerId = "test-practitioner";

        private Mock<ILogger<PractitionersController>> _loggerMock;
        private Mock<IWebHostEnvironment> _iWebHostEnvironment;

        [SetUp]
        public void SetUp()
        {
            _loggerMock = new Mock<ILogger<PractitionersController>>();
            _iWebHostEnvironment = new Mock<IWebHostEnvironment>();
        }

        [Test]
        public void GetPractitionersBySiteIdTest()
        {
            //Arrange
            ActionResult<IEnumerable<PractitionerGeneralInfo>> result = null;
            var controller = new PractitionersController(
                SetupPractitionerServiceForGetPractitionersBySiteIdAsync(),
                _loggerMock.Object,
            ...
            //Act
            Func<Task> act = async () => result = await controller.GetPractitionersBySiteId(TestSiteId);
            act.Invoke();
            ...
            var contentResult = okResult?.Value as IEnumerable<PractitionerGeneralInfo>;

            contentResult.Should()
                .NotBeNullOrEmpty();

            contentResult.FirstOrDefault()
                .Id.Should().Be(PractitionerId, "This is the one that is created in service setup");
        }

        private static IPractitionerService SetupPractitionerServiceForGetPractitionersBySiteIdAsync()
        {
            var serviceMock = new Mock<IPractitionerService>();

            serviceMock
                .Setup(x => x.GetPractitionersBySiteIdAsync(TestSiteId))
                .Returns(Task.FromResult(new List<PractitionerGeneralInfo>
                {
