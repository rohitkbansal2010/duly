// <copyright file="GeneralInfoConverterTests.cs" company="Duly Health and Care">
// Copyright (c) Duly Health and Care. All rights reserved.
// </copyright>

using Duly.CollaborationView.Encounter.Api.Repositories.Models;
using Duly.CollaborationView.Encounter.Api.Services.Mappings;
using Duly.CollaborationView.Encounter.Api.Tests.Common;
using FluentAssertions;
using NUnit.Framework;
using System.Collections.Generic;
using System.Linq;
using ApiContracts = Duly.CollaborationView.Encounter.Api.Contracts;

namespace Duly.CollaborationView.Encounter.Api.Tests.Services.Mappings
{
    [TestFixture]
    internal class GeneralInfoConverterTests : MapperConfigurator<RepositoryModelsToProcessContractsProfile>
    {
        private const string PhotoBase64Str = "";
        private const string PrimaryCarePhysicianNamePrefixTemplate = "Dr.";

        [Test]
        public void ConvertTest_WithoutRoles_WithoutPrefixes()
        {
            //Arrange
            var source = new PractitionerGeneralInfo
            {
                Id = "qwerty1",
                Names = new[]
                {
                    new HumanName
                    {
                        FamilyName = "Reyes",
                        GivenNames = new[] { "Ana", "Maria" }
                    }
                },
                Speciality = new List<string>() { "Licensed Counselor", "Behavioral Health" }
            };

            //Act
            var result = Mapper.Map<ApiContracts.PractitionerGeneralInfo>(source);

            //Assert
            result.Should().NotBeNull();

            result.HumanName.Should().BeOfType<ApiContracts.HumanName>();
            result.HumanName.Prefixes.Should().BeNull();

            result.Role.Should().Be(ApiContracts.PractitionerRole.MedicalAssistant);
        }

        [Test]
        public void ConvertTest_WithoutRoles_WithPrefixes()
        {
            //Arrange
            var source = new PractitionerGeneralInfo
            {
                Id = "qwerty1",
                Names = new[]
                {
                    new HumanName
                    {
                        FamilyName = "Reyes",
                        GivenNames = new[] { "Ana", "Maria" },
                        Prefixes = new[] { "Phd." }
                    }
                },
                Speciality = new List<string>() { "Licensed Counselor", "Behavioral Health" }
            };

            //Act
            var result = Mapper.Map<ApiContracts.PractitionerGeneralInfo>(source);

            //Assert
            result.Should().NotBeNull();

            result.HumanName.Should().BeOfType<ApiContracts.HumanName>();
            result.HumanName.Prefixes.Should().BeNull();

            result.Role.Should().Be(ApiContracts.PractitionerRole.MedicalAssistant);
        }

        [Test]
        public void ConvertTest_Physician_WithoutPrefixes()
        {
            //Arrange
            var source = new PractitionerGeneralInfo
            {
                Id = "qwerty1",
                Names = new[]
                {
                    new HumanName
                    {
                        FamilyName = "Reyes",
                        GivenNames = new[] { "Ana", "Maria" }
                    }
                },
                Roles = new[]
                {
                    new Role { Title = "Physician" }
                },
                Speciality = new List<string>() { "Licensed Counselor", "Behavioral Health" }
            };

            //Act
            var result = Mapper.Map<ApiContracts.PractitionerGeneralInfo>(source);

            //Assert
            result.Should().NotBeNull();

            result.HumanName.Prefixes.Should().NotBeNull();
            result.HumanName.Prefixes.Should().HaveCount(1);
            result.HumanName.Prefixes[0].Should().BeEquivalentTo(PrimaryCarePhysicianNamePrefixTemplate);

            result.Role.Should().Be(ApiContracts.PractitionerRole.PrimaryCarePhysician);
        }

        [Test]
        public void ConvertTest_Physician_WithPrefixes()
        {
            //Arrange
            var source = new PractitionerGeneralInfo
            {
                Id = "qwerty1",
                Names = new[]
                {
                    new HumanName
                    {
                        FamilyName = "Reyes",
                        GivenNames = new[] { "Ana", "Maria" },
                        Prefixes = new[] { "Phd." },
                        Suffixes = new[] { "II" }
                    }
                },
                Photos = new[]
                {
                    new Attachment
                    {
                        Title = "Photo",
                        ContentType = "image/x-png",
                        Data = PhotoBase64Str,
                        Size = (int)PhotoBase64Str.Length,
                        Url = "test url"
                    }
                },
                Roles = new[]
                {
                    new Role { Title = "physician" }
                },
                Speciality = new List<string>() { "Licensed Counselor", "Behavioral Health" }
            };

            //Act
            var result = Mapper.Map<ApiContracts.PractitionerGeneralInfo>(source);

            //Assert
            result.Should().NotBeNull();
            result.Id.Should().BeEquivalentTo(source.Id);

            result.HumanName.Should().BeOfType<ApiContracts.HumanName>();
            result.HumanName.FamilyName.Should().BeEquivalentTo(source.Names.First().FamilyName);
            result.HumanName.GivenNames.Should().NotBeNull();
            result.HumanName.GivenNames.Should().HaveCount(source.Names.First().GivenNames.Length);
            result.HumanName.Suffixes.Should().NotBeNull();
            result.HumanName.Suffixes.Should().HaveCount(source.Names.First().Suffixes.Length);
            result.HumanName.Prefixes.Should().NotBeNull();
            result.HumanName.Prefixes.Should().HaveCount(1);
            result.HumanName.Prefixes[0].Should().BeEquivalentTo(source.Names.First().Prefixes.First());

            result.Photo.Should().BeOfType<ApiContracts.Attachment>();
            result.Photo.Title.Should().BeEquivalentTo(source.Photos.First().Title);
            result.Photo.ContentType.Should().BeEquivalentTo(source.Photos.First().ContentType);
            result.Photo.Size.Should().Be(source.Photos.First().Size);
            result.Photo.Data.Should().BeEquivalentTo(source.Photos.First().Data);
            result.Photo.Url.Should().BeEquivalentTo(source.Photos.First().Url);

            result.Role.Should().Be(ApiContracts.PractitionerRole.PrimaryCarePhysician);
        }

        [Test]
        public void ConvertTest_Practitioner_ToParty()
        {
            //Arrange
            var source = new PractitionerGeneralInfo
            {
                Id = "qwerty1",
                Names = new[]
                {
                    new HumanName
                    {
                        FamilyName = "Reyes",
                        GivenNames = new[] { "Ana", "Maria" },
                        Prefixes = new[] { "Phd." },
                        Suffixes = new[] { "II" }
                    }
                },
                Photos = new[]
                {
                    new Attachment
                    {
                        Title = "Photo",
                        ContentType = "image/x-png",
                        Data = PhotoBase64Str,
                        Size = (int)PhotoBase64Str.Length,
                        Url = "test url"
                    }
                },
                Roles = new[]
                {
                    new Role { Title = "physician" }
                }
            };

            //Act
            var result = Mapper.Map<ApiContracts.Party>(source);

            //Assert
            result.Should().NotBeNull();
            result.Id.Should().BeEquivalentTo(source.Id);

            result.HumanName.Should().BeOfType<ApiContracts.HumanName>();
            result.HumanName.FamilyName.Should().BeEquivalentTo(source.Names.First().FamilyName);
            result.HumanName.GivenNames.Should().NotBeNull();
            result.HumanName.GivenNames.Should().HaveCount(source.Names.First().GivenNames.Length);
            result.HumanName.Suffixes.Should().NotBeNull();
            result.HumanName.Suffixes.Should().HaveCount(source.Names.First().Suffixes.Length);
            result.HumanName.Prefixes.Should().NotBeNull();
            result.HumanName.Prefixes.Should().HaveCount(1);
            result.HumanName.Prefixes[0].Should().BeEquivalentTo(source.Names.First().Prefixes.First());

            result.Photo.Should().BeOfType<ApiContracts.Attachment>();
            result.Photo.Title.Should().BeEquivalentTo(source.Photos.First().Title);
            result.Photo.ContentType.Should().BeEquivalentTo(source.Photos.First().ContentType);
            result.Photo.Size.Should().Be(source.Photos.First().Size);
            result.Photo.Data.Should().BeEquivalentTo(source.Photos.First().Data);
            result.Photo.Url.Should().BeEquivalentTo(source.Photos.First().Url);

            result.MemberType.Should().Be(ApiContracts.MemberType.Doctor);
        }

        [Test]
        public void ConvertTest_ContractPractitioner_ToParty()
        {
            var source = new PractitionerGeneralInfo
            {
                Id = "qwerty1",
                Names = new[]
                {
                    new HumanName
                    {
                        FamilyName = "Reyes",
                        GivenNames = new[] { "Ana", "Maria" },
                        Prefixes = new[] { "Phd." },
                        Suffixes = new[] { "II" }
                    }
                },
                Photos = new[]
                {
                    new Attachment
                    {
                        Title = "Photo",
                        ContentType = "image/x-png",
                        Data = PhotoBase64Str,
                        Size = (int)PhotoBase64Str.Length,
                        Url = "test url"
                    }
                },
                Roles = new[]
                {
                    new Role { Title = "MedicalAssistant" }
                },
                Speciality = new List<string>() { "Licensed Counselor", "Behavioral Health" }
            };
            var practitionerContract = Mapper.Map<ApiContracts.PractitionerGeneralInfo>(source);

            var resultFromSource = Mapper.Map<ApiContracts.Party>(source);
            var resultFromContract = Mapper.Map<ApiContracts.Party>(practitionerContract);

            resultFromSource.Should().BeEquivalentTo(resultFromContract);
        }
    }
}
