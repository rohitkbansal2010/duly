// <copyright file="ScheduleFollowupRepositoryTests.cs" company="Duly Health and Care">
...
{
    [TestFixture]
    public class ScheduleFollowupRepositoryTests
    {
        ...
        {
            var scheduleFollowUp = new Models.CheckOut.ScheduleFollowUp
            {
            ...
            var schFollowupDetails = SetupPostScheduleFollowupDapperContext();

            var repositoryMock = new ScheduleFollowupRepository(_dapperContextMock.Object);

            var results = await repositoryMock.PostScheduleFollowup(scheduleFollowUp);

            //Assert
            Assert.AreEqual(results, schFollowupDetails);
        }
        ...
        private int SetupPostScheduleFollowupDapperContext()
        {
            int res = 2;

            _dapperContextMock
                .Setup(context =>
                    context.ExecuteScalarAsync<int>(PostScheduleFollowUpStoredProcedureName, It.IsAny<object>(), default, default))
                .ReturnsAsync(res);
